...
static inline uint64_t ror(uint64_t n, int k) { return (n >> k) | (n << (64 - k)); }
...
char *sha512_crypt(const char *key, const char *setting, char *output, size_t out_len) {
    if (!key || !setting || !output || out_len < 128) return NULL;
    ...
    if (strncmp(salt, "rounds=", 7) == 0) {
        salt += 7;
        if (!isdigit(*salt)) return NULL;
        char *end = NULL;
        unsigned long u = strtoul(salt, &end, 10);
        if (*end != '$') return NULL;
        salt = end + 1;
        if (u < ROUNDS_MIN) r = ROUNDS_MIN;
        else if (u > ROUNDS_MAX) r = ROUNDS_MAX;
        else r = (unsigned int)u;
        if (r != ROUNDS_DEFAULT) {
...
bool sha512_crypt_gensalt(char *out, size_t out_len) {
    if (!out || out_len < 32) return false;
    ...
    char salt[17];
    for (int i = 0; i < 16; i++) {
        salt[i] = b64[rand_bytes[i] % 64];
    }
    salt[16] = '\0';
    snprintf(out, out_len, "$6$%s$", salt);
    return true;
}
...
bool sha512_crypt_verify(const char *key, const char *hash) {
    ...
    char computed[130];
    if (!sha512_crypt(key, hash, computed, sizeof(computed))) return false;

    size_t len_hash = strlen(hash);
    size_t len_comp = strlen(computed);
    if (len_hash != len_comp) return false;

    volatile uint8_t diff = 0;
    for (size_t i = 0; i < len_hash; i++) {
        diff |= ((uint8_t)hash[i] ^ (uint8_t)computed[i]);
    }
    return (diff == 0);
}
